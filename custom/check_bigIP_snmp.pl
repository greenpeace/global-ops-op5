...
#
# Example: ./check_bigIP_snmp.pl 192.168.101.100 public 20 10
#
...
if (!$ARGV[2]) { $warn="20";} else {$warn=$ARGV[2];}
if (!$ARGV[3]) { $crit="10";} else {$crit=$ARGV[3];}
...
if ($retval < $warn) {
        if ($retval < $crit) {
                print "CRITICAL - CPU Free = " . $retval . "\n";
                exit $STATE_CRITICAL;
                }
        print "WARNING - CPU Free = " . $retval . "\n";
        exit $STATE_WARNING;
}

print "OK - CPU Free = " . $retval . "\n";
exit $STATE_OK;
