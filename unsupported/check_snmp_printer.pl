...
if ( $ptype == 1 ) {   # STD MIB
	...
	my %std_mib = (
		...
		std_mib_alert_location => 				".1.3.6.1.2.1.43.18.1.1.6",
	...
	my %std_mib_prt_alert_code = (
		...
		40000 => "unknown1 (Brother printer)",
		10001 => "unknown2 (Brother printer)",
	);
	...
	$display .= $snmp_response{$std_mib{'std_mib_status_display'} . ".1.1" } . " "; # Only get the .1.1 one, because .1.2 is a hex string (which seems to tell the resolution)
	...
	if (defined ( $std_mib_inst_count{'std_mib_alert_sev_level'} )  ) {
		...
		# During power-save and warming, just report OK
...
# There seems to be no reason for the below line, in fact it seems to break things, so try without it (keeping the commented line in case anyone knows a reason for it and how to make it work with it uncommented, please include a reason if uncommented)
